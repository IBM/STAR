...
def main():
    ...
    for task, tv_dict in tv_dicts.items():
        ...
        if task == 'stsb':
            spearman_rho = evaluators[task].evaluate_stsb(lora_model, tokenizer, device=args.device, print_output=False,  batch_size = 16)        
            print(f'{task}: Spearman_rho : {round(spearman_rho, 4)}')
            accuracy_records[task] = round(spearman_rho, 4)
        else:
            accuracy = evaluators[task].evaluate(lora_model, tokenizer, device=args.device, print_output=False,  batch_size = 16)
